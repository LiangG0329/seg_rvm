...
class Evaluator:
    ...
    def evaluate_worker(self,  clip, position):
        ...
        for i, framename in enumerate(tqdm(framenames, desc=f'{clip}', position=position, dynamic_ncols=True)):
            ...
            true_pha = cv2.imread(os.path.join(self.args.true_dir,  clip, 'pha', framename), cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255
            ...
            pred_pha = cv2.imread(os.path.join(self.args.pred_dir,  clip, 'pha', framename), cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255
            ...
            if 'fgr_mse' in self.args.metrics or 'fgr_mad' in self.args.metrics:
                ...
                true_fgr = cv2.imread(os.path.join(self.args.true_dir,  clip, 'fgr', framename), cv2.IMREAD_COLOR).astype(np.float32) / 255
                #pred_fgr = cv2.imread(os.path.join(self.args.pred_dir, dataset, clip, 'fgr', framename),cv2.IMREAD_COLOR).astype(np.float32) / 255
                pred_fgr = cv2.imread(os.path.join(self.args.pred_dir,  clip, 'fgr', framename), cv2.IMREAD_COLOR).astype(np.float32) / 255
                true_msk = true_pha > 0
