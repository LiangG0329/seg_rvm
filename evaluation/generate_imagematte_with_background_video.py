...
def process(i):
    ...
    image_files = os.listdir(os.path.join(args.background_dir, background_filename))
    bgrs = []
    for image_file in image_files:
        #print(image_file)
        with Image.open(os.path.join(args.background_dir, background_filename, image_file)) as bgr:
            bgr = bgr.convert('RGB')
            bgrs.append(bgr)
    #print(len(bgrs))
