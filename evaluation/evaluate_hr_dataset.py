...
class Evaluator:
    ...
    def evaluate(self):
        ...
        with ThreadPoolExecutor(max_workers=self.args.num_workers) as executor:
            #for dataset in sorted(os.listdir(self.args.pred_dir)):
                if os.path.isdir(os.path.join(self.args.pred_dir)):
    ...
    def evaluate_worker(self, clip, position):
        framenames = sorted(os.listdir(os.path.join(self.args.pred_dir, clip, 'pha')))
        ...
        for i, framename in enumerate(
                tqdm(framenames, desc=f'{clip}', position=position, dynamic_ncols=True)):
            true_pha = cv2.imread(os.path.join(self.args.true_dir, clip, 'pha', framename),
                                  cv2.IMREAD_GRAYSCALE)
            pred_pha = cv2.imread(os.path.join(self.args.pred_dir,  clip, 'pha', framename),
                                  cv2.IMREAD_GRAYSCALE)
            ...
            if 'fgr_mse' in self.args.metrics:
                true_fgr = cv2.imread(os.path.join(self.args.true_dir,  clip, 'fgr', framename),
                                      cv2.IMREAD_COLOR)
                pred_fgr = cv2.imread(os.path.join(self.args.pred_dir,  clip, 'fgr', framename),
                                      cv2.IMREAD_COLOR)
                ...
                device = 'cpu'
                true_msk = true_msk.to(device)  # 将 true_msk 张量移动到主设备（设备类型由 device 变量确定）
                pred_fgr = pred_fgr.to(device)  # 将 pred_fgr 张量移动到主设备
                true_fgr = true_fgr.to(device)  # 将 true_fgr 张量移动到主设备
...
class MetricGRAD:
    ...
    def gauss_gradient(self, img):
        ...
        # img_filtered_x = cv2.filter2D(img[None, None, :, :], self.filter_x, border_type='replicate')[0, 0]
        img_filtered_y = kornia.filters.filter2d(img[None, None, :, :], self.filter_y, border_type='replicate')[0, 0]
        # img_filtered_y = cv2.filter2D(img[None, None, :, :], self.filter_y, border_type='replicate')[0, 0]
