...
class Trainer:
    ...
    def init_model(self):
        ...
        self.model = MattingNetwork_(pretrained_backbone=True).to(self.rank)
        #self.model = MattingNetwork_(pretrained_backbone=False).to(self.rank)

        if self.args.checkpoint:
            self.log(f'Restoring from checkpoint: {self.args.checkpoint}')
            self.log(self.model.load_state_dict(
                torch.load(self.args.checkpoint, map_location=f'cuda:{self.rank}'), strict=True))
