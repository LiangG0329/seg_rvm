...
from inference_utils import VideoReader, VideoWriter, ImageSequenceReader, ImageSequenceWriter, ImageReader, \
    ConstantImage
    ...
    try:
        with torch.no_grad():
            ...
            for index, src in enumerate(reader):
                ...
                # print(type(src))
                # fgr, pha, res, re, *rec = model(src, *rec, downsample_ratio)
                #fgr, pha, hid, *rec = model(src, *rec, downsample_ratio)
                fgr, pha, *rec = model(src, *rec, downsample_ratio)  # init
                #for re in rec:
                #    print(re.shape)
                ...
                if output_composition is not None:
                    if output_type == 'video':
                        ...
                        fgr = fgr * pha.gt(0)
                        bgr = torch.tensor([120, 255, 155], device=device, dtype=dtype).div(255).view(1, 1, 3, 1, 1)  # del  # del
                        #com = torch.cat([fgr, pha], dim=-3)  # RGBA png images
                        com = fgr * pha + bgr * (1 - pha)  # del
                    writer_com.write(com[0])

                #for i in range(0, 1):
                if output_rec is not None:
                    #for i in range(0, 16):
                    #h = hid[0][0][0].unsqueeze(dim=0) # png
                    #B, T, C, H, W = hid.shape[:]
                    #h = hid[:, :, 1, :, :].reshape(B, T, 1, H, W)  # video
                    #h = hid[:, :, 15:16, :, :]
                    #for r in rec:
                    #    print(r.shape)
                    re = rec[0][:, 15:16, :, :]  # rec hidden state
                    if (re.size(2) % 2 != 0):
                        re = re[:, :, :-1, :]
                    if (re.size(3) % 2 != 0):
                        re = re[:, :, :, :-1]
...
class Converter:
    ...
    def __init__(self, checkpoint: str, device: str):
        ...
        self.model = MattingNetwork_().eval().to(device)
